...
class RedisStateManager:
    ...
    def _publish(self, session_id: str, file_id: str):
        ...
        try:
            self.r.publish(RK.progress_ch(session_id), json.dumps(event))
        except Exception as e:
            logger.warning("Redis publish (file_progress) failed: %s", e)
        if fp.stage in ("done", "skipped", "error"):
            self._try_complete_session(session_id)
    ...
    def _try_complete_session(self, session_id: str):
        ...
        if summary and summary["in_progress"] == 0:
            ...
            try:
                ...
                logger.warning("Redis publish (session_complete) failed: %s", e)
